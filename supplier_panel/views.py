...
#                 order_count_list = []
#                 for i in range(1, 13):
#                     product_count = Product.objects.all().filter(date__year=today.year, date__month=i,).count()
#                     product_count_list.append(product_count)
...
#             profile_obj = None
#
#         if profile_obj != None and profile_obj.status == "vendor":
#
#             try:
...
#         else:
#             messages.warning(
#                 request, 'Non-suppliers are not allowed to access this page')
#
...
#             bank_info_obj =BankAccount.objects.get(vendor_profile__user=request.user,)
#             context = {
...
#                 old_social_links = SocialLink.objects.get(vendor_profile=profile,)
...
#             social_links_obj =SocialLink.objects.get(vendor_profile__user=request.user,)
#             context = {
...
#         bank_info_obj = BankAccount.objects.filter(vendor_profile__user = request.user).first()
#
